...
- AI：后端 Workspace、Orchestrator、Skill、Tool、草稿审批与可选模型 provider
...
Culina/
  ...
  docs/       # 前端、后端、AI 助手开发规范；plans/ 保存设计和任务清单
...
| `npm run backend:typecheck` | 运行后端 Python 语法编译检查 |
| `npm run backend:quality` | 运行后端语法编译检查和 pytest |
...
当前后端质量入口先使用标准库 `compileall` 和 pytest，不额外引入 lint 依赖；后续若采用 ruff，应在 `backend/requirements.txt` 和项目配置中单独加入并更新 `backend:quality`。
