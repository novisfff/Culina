...
第一条路径中的候选卡不是正式 Draft：它不创建 `AITaskDraft` 或 `AIApprovalRequest`，不会让 run 进入 `waiting_approval`，前端只用本地状态保存勾选结果。用户点击“按选中项准备入库”后，前端再伪造一条新的用户消息启动下一次 run。结果是候选解析、用户澄清和正式审批没有形成一个可恢复的状态机。
...
这不是单个工具报错问题，而是 Skill 归属、解析合同、暂停语义和 Draft 类型同时不统一。本设计将所有库存增加收敛到 `inventory_analysis` 和正式 `inventory_intake` Draft，并删除旧候选卡与旧 `shopping_intake` AI 合同。
...
5. Draft 出现后 run 必须进入 `waiting_approval`，不得继续对话、调用工具或显示第二套候选 UI。
...
- Draft 创建后当前 run 必须停止在 `waiting_approval`；
...
`inventory_intake` 必须复用现有正式 Draft 渲染和审批状态：

- 后端创建 `AITaskDraft` 和 `AIApprovalRequest`；
- 消息中呈现标准 Draft part，而不是 result card；
- run 状态为 `waiting_approval`；
- 前端不维护第二份候选本地状态；
- 主按钮调用标准审批提交 API；
- 用户批准、修改或拒绝前，不产生后续模型消息。
...
- 全部 skip 不能提交；
- Draft 创建后 run 为 `waiting_approval`。
...
- 非法 action 切换不可用；
- 单一主审批按钮；
- Draft 出现后没有旧候选卡或 product-loop 按钮；
- waiting、批准、拒绝、stale 和失败状态；
- 手机端滚动、展开、日期和数量编辑；
- 键盘操作、焦点顺序、错误提示和读屏标签。
...
| 多行无阻断项 | 只生成一份 `inventory_intake` Draft，终态为 `waiting_approval` |
...
run 停在 `waiting_approval`。用户一次确认后，鸡蛋、三文鱼、牛奶库存与两个采购项在一个事务中更新；任何一行失败时全部不变。
...
6. Draft 出现后 run 进入 `waiting_approval`，不继续执行模型对话。
