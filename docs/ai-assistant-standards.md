...
`ToolCallingSkill` 负责暴露工具白名单、执行脚本和业务 Tool、通过 `generate_with_tools()` 让模型在白名单内自主选择工具、捕获 draft tool 的真实输出、校验卡片类型、草稿类型和最终结构化结果。

`WorkspaceGraphRunner` 顺序执行 Planner 选择的 Skill，并负责运行状态、SSE 进度、消息持久化、draft 持久化和审批中断。不要因为前端时间戳相同就假设后端并行执行多个 Skill 或 Tool。
