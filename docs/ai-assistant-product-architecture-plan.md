...
- Planning Layer
  - 负责任务理解、槽位抽取、缺失信息判断、skill 选择和步骤拆解。
  - 可以使用模型输出结构化 PlannerResult，但后端必须规范化和校验。
  - 不直接提交数据库事务，不实现具体业务生成逻辑。

- Skill Layer
  - 放库存、菜谱、餐食计划、购物清单、食物资料、生图等业务 skill。
  - 每个 skill 专注一个任务域，通过 manifest 声明输入、输出、工具和审批策略。
...
```text
backend/app/ai/
  runtime/       # 通用 provider、runner、event schema
  planning/      # WorkspacePlanner、意图理解、任务拆解、状态规划
  skills/        # 面向业务域的 skill manifest 和 skill handler
  tools/         # 受控业务工具 registry、executor、handler
  agents/        # 兼容旧生成函数，逐步迁移到 skills
...
- `ai/runtime`
  - 放 provider、runner、通用 schema。
  - 不 import 业务模型，不写数据库，不拼业务 prompt。

- `ai/planning`
  - 放总控 Planner、意图理解、槽位规范化、任务拆解、conversation task state 规划。
  - 可以读取 skill manifest，但不直接写业务表，也不实现具体业务生成逻辑。

- `ai/skills`
  - 放按业务域拆分的 skill，例如 inventory、recipe、meal_plan、shopping、food_profile。
  - AI 工作台的业务能力都应放在这里，围绕跨模块任务设计。

- `ai/tools`
  - 放真实工具 handler、Tool Registry 和 ToolExecutor。
  - 工具负责权限、家庭隔离、输入输出校验和 tool call 记录。

- `ai/agents`
  - 仅保留未完全迁移前的兼容生成函数。
  - 后续具体业务能力应逐步迁移到 `ai/skills`。
...
- 工作台级 AI 能力
  ...
  - 工作台入口走 `ai/planning`、`ai/skills` 和 `ai/tools`，不把页面级 assistant 作为过渡核心。

- 业务 skill 建设
  - 根据目标任务直接设计 inventory、recipe、meal_plan、shopping、food_profile、image skill。
  - 可以参考页面级能力中的 context loader、formatter、provider 调用方式，但不要复制页面 mode 驱动的设计。
  - 跨模块任务从一开始交给 Planner 拆解，再由 SkillExecutor 串联执行。
...
系统级 AI 工作台应从一开始设计 skill registry，避免工作台能力继续堆成单一页面助手或关键词路由器。Skill 是内部任务能力层，不是外部插件系统。
...
- Planner 可以根据用户目标和后端配置找到候选 skill。
- API 或调试工具可以返回当前系统支持的 AI 能力。
- 测试可以枚举所有 skill 并检查基础约束。
- 第一阶段先注册 `today_recommendation`、`recipe_draft`、`fallback_chat`、`inventory_analysis`、`meal_plan`、`shopping_list`、`meal_log`、`food_profile`，后续再增加生图和营养分析等 skill。
...
第一阶段工具必须是真实 handler，不只是工具名称清单。AI 工作台应通过 ToolExecutor 记录真实 tool call，避免能力继续堆在页面级函数或 skill 内部散落 SQL。
...
- 已引入 run events，并在 AIApplicationService 主链路中记录 intent、context、agent、finalize、cancel 等用户可读进度事件。
- 已新增 `/api/ai/chat/stream` 与 `/api/ai/runs/{run_id}/events/stream` SSE 通道，保留 `/api/ai/chat` 兼容入口。
- 前端已展示执行进度时间线，发送中也会展示流式进度条。
- 已支持长任务取消、失败重试、局部重生成。
- 多轮对话已能围绕同一个草稿持续修改，当前草稿会作为 `currentDraft` 注入后续同会话请求。

验收：

- 用户能看到 AI 正在执行哪一步：由 `RunProgressTimeline` 与 chat stream progress 事件覆盖。
- 长任务失败后可以重试：由 `/api/ai/runs/{run_id}/retry` 与消息气泡“重试这次任务”覆盖。
- 用户可以说“第二天不要吃鸡肉”并修改已有菜单草稿：由同会话 `currentDraft` 注入与 meal_plan 修改路由覆盖。
