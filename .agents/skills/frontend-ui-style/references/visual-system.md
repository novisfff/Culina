...
- `--control-height*` 固定控件可见高度，`--tap-min` 固定独立交互目标的实际命中区域；图标或文字可以更小，但不得用缩小命中区域维持拥挤布局。
- 桌面普通按钮、输入、select 和独立图标按钮使用 `--control-height`；鼠标环境中的低频工具栏、次级图标工具和低频筛选可以使用 `--control-height-compact`。
- 手机、平板和 `pointer: coarse` 设备上的独立交互目标不得小于 `--tap-min`；高频按钮、输入和选择控件使用 `--control-height-touch`。
- compact 不能用于手机或平板、主操作、关闭、返回、危险操作和唯一恢复动作。空间不足时改为换行、纵向排列、缩短文案或收起低频操作，不把目标压小。
- 默认图标视觉尺寸固定 18px；关闭、返回和强调图标固定 20px。独立图标按钮的外层命中区域使用 44px，视觉图标不随热区放大。
- 正文中的行内链接、非交互 badge、装饰图标、分割线和纯展示状态不适用 `--tap-min`；行内链接仍需保持可读行高、清楚焦点和与相邻链接的可辨间距。
- 用透明 padding、伪元素或外层按钮扩大命中区域时，区域必须与可见控件位置一致，不得与相邻目标重叠，也不能遮挡卡片或图片的其他操作。
