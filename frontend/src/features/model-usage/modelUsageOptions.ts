...
> = {
  llm: { label: '文本与视觉理解', description: '对话、菜谱建议和图片理解' },
  embedding: { label: '语义索引', description: '让搜索理解食材和菜谱语义' },
  rerank: { label: '结果重排', description: '优化搜索结果的相关性' },
...
export const MODEL_USAGE_METER_OPTIONS: Record<ModelUsageMeter, { label: string }> = {
  ...
  total_tokens: { label: '总 Token' },
  embedding_tokens: { label: '向量 Token' },
  rerank_requests: { label: '重排请求' },
  rerank_documents: { label: '重排文档' },
...
> = {
  ...
  capability_degraded: { label: '部分功能可能降级', message: '部分模型能力可能暂时使用基础方式处理。' },
  measurement_unavailable: { label: '计量服务暂时异常', message: '当前无法完整确认模型额度，请稍后重试。' },
};
...
export const MODEL_USAGE_ERROR_OPTIONS: Record<ModelUsageErrorCode, { title: string; message: string }> = {
  ...
  model_usage_future_period_not_allowed: { title: '账期尚未开始', message: '请选择当前或已结束的月份。' },
  model_usage_guardrail_quantity_unavailable: { title: '暂时无法确认这项用量', message: '为避免超出限制，本次没有发起模型调用。' },
  model_usage_historical_rollup_not_found: { title: '历史统计暂不可用', message: '该账期的汇总数据尚未生成。' },
  model_usage_invalid_group_by: { title: '统计方式不可用', message: '请刷新后重新选择统计方式。' },
  model_usage_invalid_period: { title: '账期格式不正确', message: '请选择一个有效的月份。' },
  model_usage_ledger_unavailable: { title: '暂时无法确认模型额度', message: '请稍后重试；当前没有发起新的模型调用。' },
  model_usage_missing_price_confirmation_required: { title: '请确认缺价影响', message: '开启硬限制前，请先确认缺少价格的调用会被阻止。' },
  model_usage_policy_conflict: { title: '预算设置已更新', message: '请查看最新设置后再应用当前修改。' },
  model_usage_policy_validation_error: { title: '预算设置无法保存', message: '请检查预算和能力护栏后再试。' },
