    ...
    <main className={`model-usage-workspace model-usage-request-logs-page ${props.isPhoneViewport ? 'is-mobile' : ''}`}>
      <header className="model-usage-request-page-header">
        ...
        <div className="model-usage-request-page-copy">
          <p>模型用量明细</p>
          <h1>请求日志</h1>
          <small>{isFamilyScope ? '按日期、模型和核对状态查看家庭请求。' : '按日期、能力和核对状态查看我的请求。'}</small>
        </div>
      </header>
      <section className="model-usage-request-filters" aria-label="请求日志筛选">
        <div className="model-usage-request-filters-head">
          <div>
            <h2>筛选请求</h2>
            <p>{isFamilyScope ? '按时间和模型条件定位记录' : '按时间和能力条件定位记录'}</p>
          </div>
          {logs.isOwner ? (
            <div className="model-usage-scope-toggle" aria-label="日志范围">
        ...
        <div className="model-usage-request-filter-actions">
          <button type="button" onClick={logs.actions.resetFilters}>清除条件</button>
          <button type="button" onClick={logs.actions.applyFilters}>查询记录</button>
        </div>
      </section>
      {requestQuery.isLoading && !logs.page ? <p role="status">正在加载请求日志。</p> : <ModelUsageRequestLogs page={logs.page} />}
      {logs.page ? (
        <nav className="model-usage-request-pagination" aria-label="请求日志分页">
