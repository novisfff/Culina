...
describe('inventory maintenance route styles', () => {
  it('loads inventory-only styles from the lazy overlay entry', () => {
    ...
    const entrySource = readFileSync(resolve(__dirname, 'InventoryMaintenanceDialogs.tsx'), 'utf8');
    expect(globalStyles).not.toContain("@import './styles/10-inventory-actions.css' layer(domain);");
    expect(globalStyles).not.toContain("@import './styles/11-inventory-maintenance.css' layer(domain);");
    ...
    expect(entrySource).toContain("./inventory-route.css");
  });
});
