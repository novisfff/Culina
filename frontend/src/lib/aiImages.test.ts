...
describe('ai image helpers', () => {
  ...
  it('returns pending image job ids only for active jobs', () => {
    expect(getPendingImageJobId({ pendingJob: { job_id: 'image-job-1', status: 'running', generation_mode: 'text', can_retry: false } })).toBe('image-job-1');
    expect(getPendingImageJobId({ pendingJob: { job_id: 'image-job-2', status: 'succeeded', generation_mode: 'text', can_retry: false } })).toBeNull();
