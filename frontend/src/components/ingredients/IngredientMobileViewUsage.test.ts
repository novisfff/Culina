...
describe('IngredientMobileView shared overlay usage', () => {
  ...
  it('mobile ingredient page mixes ready food stock into inventory library', () => {
    ...
    expect(mobileSource).toContain("label: '有库存'");
    expect(mobileSource).toContain("label: '需要补充'");
    ...
    expect(mobileSource).toContain('<h2>今天需要处理 <span>{priorityItemCount} 项</span></h2>');
