...
const COOK_FINISH_STEPS: Array<{ id: CookFinishStepId; label: string; description: string }> = [
  ...
  { id: 'meal', label: '餐食记录', description: '选择日期、餐次和同步记录' },
...
export function RecipeCookFinishDialog(props: RecipeCookFinishDialogProps) {
  ...
  function skipCurrentStep() {
    ...
    if (activeStep.id === 'meal') {
      props.onUpdateSession({ createMealLog: false });
    }
    if (activeStep.id === 'feedback') {
  ...
  function renderMealStep() {
    return (
      <StepPanel
        title="决定是否同步餐食记录"
        description="日期和餐次会用于本次做菜记录；开启同步后，也会生成一条餐食记录。"
      >
