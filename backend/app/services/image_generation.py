...
def RECIPE_PROMPT_BUILDER(request: ImageGenerationRequest) -> str:
    ...
    return "\n".join(
        [
            "为菜谱生成一张专用于做菜页面顶部的横向 Banner 背景图，突出真实成菜状态，同时保持克制、温暖和家庭感。",
            *detail,
            "构图必须像 UI banner：左侧 55% 是浅色干净留白，右侧 35% 放成菜或关键食材，主体靠右下，允许画面边缘有轻柔虚化。",
            "画面使用暖色自然光、奶油白或浅暖色家庭厨房/餐桌背景，整体明亮通透，适合被裁切成长条圆角 Banner。",
            "不要居中摆盘，不要卡片封面构图，不要把主体放到左侧文字安全区。",
