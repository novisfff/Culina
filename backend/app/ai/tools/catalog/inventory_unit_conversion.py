...
def normalize_unit_mismatch(value: dict[str, Any]) -> dict[str, Any]:
    ...
    if not ingredient_id or not ingredient_name or not default_unit or not unsupported_unit:
        raise ValueError("单位换算请求缺少食材或单位信息")
    ...
    if matched_operation is None:
        raise ValueError("单位换算请求与原始库存操作不匹配")
...
def unit_mismatch_from_tool_payload(value: dict[str, Any]) -> dict[str, Any]:
    unit_mismatch = value.get("unitMismatch") if isinstance(value.get("unitMismatch"), dict) else None
    if unit_mismatch is None:
        raise ValueError("单位换算草稿缺少原始单位信息")
    return normalize_unit_mismatch(unit_mismatch)


def build_unit_conversion_candidate(
    *,
    unit_mismatch: dict[str, Any],
    ratio_to_default: Decimal,
    source_message: str,
) -> dict[str, Any]:
    return {
        "type": "unit_conversion_candidate",
        "ingredientId": unit_mismatch.get("ingredientId"),
        "ingredientName": unit_mismatch.get("ingredientName"),
        "defaultUnit": unit_mismatch.get("defaultUnit"),
        "unit": unit_mismatch.get("unsupportedUnit"),
...
def build_unit_mismatch_inventory_payload(
    ...
    *,
    ...
    ingredient_id = str(unit_mismatch.get("ingredientId") or "").strip()
    ...
    original_draft = unit_mismatch.get("originalDraft") if isinstance(unit_mismatch.get("originalDraft"), dict) else {}
    ...
    unsupported_unit = normalize_unit_label(str(unit_mismatch.get("unsupportedUnit") or ""))
    default_unit = normalize_unit_label(str(unit_mismatch.get("defaultUnit") or ingredient.default_unit))
