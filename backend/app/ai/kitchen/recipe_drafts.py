    ...
    "properties": {
        ...
        "ingredient_items": {
            ...
            "items": {
                "type": "object",
                "additionalProperties": False,
                "required": ["ingredient_id", "ingredient_name", "note"],
                "properties": {
...
def _selected_ingredient_lines(context: AgentContext) -> list[str]:
    def unit_context(item: Ingredient) -> str:
        ...
        quantity_mode = item.quantity_tracking_mode.value if hasattr(item.quantity_tracking_mode, "value") else str(item.quantity_tracking_mode)
        parts = [f"默认单位={item.default_unit}", f"计量模式={quantity_mode}"]
        if supported_units:
...
- 对 `quantityTrackingMode=track_quantity` 的食材，ingredient_items 中的 quantity 和 unit 必须已经换算到该食材的默认单位；用户给出支持单位时按配置换算后填写默认单位。
- 对 `quantityTrackingMode=not_track_quantity` 的食材，不需要硬填结构化 quantity/unit；把“少许、喷油、按口味、出锅前撒”等用量线索写在步骤 text 或 note 里。
...
- 数量要按 servings 估算，不能都写 1 或“适量”；但 `quantityTrackingMode=not_track_quantity` 的主要调料可以不填结构化数量，必须在步骤或备注里给可执行用量线索。
