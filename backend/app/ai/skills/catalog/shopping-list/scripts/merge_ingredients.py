...
def merge_ingredients(items: list[dict]) -> list[dict]:
    """Normalize and merge shopping items that share the same title and unit."""
    merged: dict[tuple[str, str], dict] = {}
    for item in items:
        title = _normalize_ingredient(item.get("title") or item.get("name") or "")
    ...
    return [
        {"title": value["title"], "quantity": float(value["quantity"]), "unit": value["unit"]}
