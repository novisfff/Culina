...
1. 读取待采购项和可用库存。
2. 根据用户明确需求整理采购项。
3. 对具体食材名称调用 `ingredient.search` / `ingredient.read_by_id`，能明确判断是同一食材时绑定真实 `ingredientId`；不能匹配时不要创建购物清单草稿。
4. 如果存在缺失食材档案，先普通回复说明本次缺哪些食材、为什么要先建档、会逐项生成可确认食材草稿；随后请求/注入 `ingredient_profile` 并逐个生成 `ingredient_profile.create_draft`，等用户确认食材档案后再继续采购清单。
...
7. 每个采购项都有真实 `ingredientId` 后，调用 `shopping.create_draft`。
...
5. 每个采购项都有真实 `ingredientId` 后，使用真实 `sourceDraftId` 调用 `shopping.create_draft`。`sourceDraftId` 只能是当前运行 artifact 的 `in_run:*` ID，或同一会话中真实存在的持久草稿 ID。

## 修改购物清单

1. 优先通过 `shopping.read_pending` 或 `shopping.read_by_id` 定位真实购物项。
2. 修改、标记买到、恢复待买或删除正式购物项时，生成 `operations` 草稿；每项必须带真实 `targetId`、`baseUpdatedAt` 和可编辑 payload；create/update payload 必须绑定真实 `ingredientId`。
