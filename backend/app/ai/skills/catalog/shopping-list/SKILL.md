...
- 用户明确要买某个食材且 `ingredient.search` 有明确同一食材候选时，采购项应绑定真实 `ingredientId`；无匹配时可保留自由 `title`，但它只是购物提醒，不是食材档案。
- 从 meal_plan artifact 派生时，缺失食材已有真实 `ingredientId` 就绑定；没有 ID 的只作为采购名称，不能反向创建或假装创建食材档案。
