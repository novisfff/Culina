...
- 创建采购清单前，每个采购项都必须绑定当前家庭真实食材 ID；`ingredient.search` 只是候选召回，不能把检索结果自动当作匹配成功。
...
- 用户明确要买某个食材且 `ingredient.search` 有明确同一食材候选时，采购项必须绑定真实 `ingredientId`；无匹配时不要调用 `shopping.create_draft`。
- 从 meal_plan artifact 派生时，缺失食材已有真实 `ingredientId` 就绑定；没有 ID 时必须先引导创建食材档案，不能只作为采购名称写入清单。

## 执行规则

- 创建前读取待采购项和可用库存。
- 涉及具体食材名称、单位或常见别名时，优先查询真实食材资料；没有匹配食材时不要编造食材 ID，也不要保留自由采购项。
- 发现一个或多个采购项缺少真实食材档案时，先用普通回复说明整体情况：哪些食材缺档案、为什么必须先建档、接下来会逐项生成可确认食材档案草稿；说明后请求/注入 `ingredient_profile`，通过 `ingredient_profile.create_draft` 逐个创建食材档案，用户确认后再继续生成采购清单草稿。
...
- 仅通过 `shopping.create_draft` 生成 `shopping_list` 草稿，且每个 create/update payload 必须带真实 `ingredientId`。
