...
- 采购前计划和待办维护；任何“买到了 / 按小票入库 / 完成并入库 / 朋友送的入库”应路由到 `inventory_analysis`。
...
- `shopping.create_draft` 不得生成 `set_done(done=true)`；Tool 会拒绝并要求改由 `inventory_analysis` 处理。
- `quantityMode` 只能是 `track_quantity` 或 `not_track_quantity`。presence-only 采购不硬填精确数量。

### 采购完成与入库

- 用户说“这些买到了”“按小票入库”“完成并入库”“朋友送的入库”，或“实际只买了 / 剩下继续待买”的部分采购时，不要继续本 Skill 的草稿；路由或注入 `inventory_analysis`。
...
- 本 Skill 不再声明任何采购完成入库 handoff；“买到了”统一由 `inventory_analysis` 处理。

## 审批规则

- 普通 create/update/delete/restore 仅通过 `shopping.create_draft` 生成 `shopping_list` Draft；模型始终只生成 Draft。draft_then_confirm 等待真实用户决定；draft_then_policy 只生成 Draft，服务端在 evidence/authorization/allowlist/limits/version/revert-adapter 全通过才提交，否则降级人工确认。Composite/Continuation 始终人工确认。
...
- 用户要完成采购时，说明将进入库存 Skill 做统一入库确认。
