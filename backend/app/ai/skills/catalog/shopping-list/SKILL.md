...
- `shopping_completed_ingredient` / `shopping_completed_food`：只有一个 `set_done.payload.done=true` 操作审批并提交成功后，后端才用已提交购物项生成 `shopping_to_stock.v1` continuation；恢复待买、更新、删除、拒绝、提交失败或一次完成多个项目都不触发。
