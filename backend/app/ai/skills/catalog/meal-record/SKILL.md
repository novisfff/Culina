...
- `update`：补充详情使用 `update_details`，评分使用 `rate_food`，纠正实际吃了什么、份数或条目备注使用 `update_composition`；先读取真实记录，新增 Food 还必须逐个通过 `food.search` / `food.read_by_id` 绑定当前家庭真实对象。
...
- `update_composition` 必须提交完整最终 Food 列表，现有条目保留真实 `entryId`，新增条目不填 `entryId`；至少保留一个 Food，同一餐不能重复 Food。组成纠错只修改 MealLog，不补回、追加或重新计算历史库存，也不改变关联计划事实。
...
- 仅用于识别或理解的图片不写入 `mediaIds`；只有用户明确要求作为本次用餐照片保存时才绑定。
...
- 确认或策略提交前不得写正式 MealLog；Food 缺失或关键 ID 不真实时不得生成无效草稿。
...
- 组成纠错草稿必须展示更新前后 Food、份数和备注差异，并明确说明不会补回、追加或重新计算历史库存，也不会改变关联计划事实。
