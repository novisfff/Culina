...
- `storage_location` 只用于成品、速食、包装食品，优先 `冷藏`、`冷冻`、`常温`；保存位置不明确时留空或追问，不默认常温。
...
- 接收 `shopping_completed_food` 时，从 `shopping_to_stock.v1` 取得精确 `foodId`，重新调用 `food.read_by_id` 校验当前家庭 ready-like Food；向用户展示采购数量和单位，按“现有库存数量 + 本次采购数量”生成第二份 `food_profile` 更新草稿，并保留重读到的其他资料字段。
- 购物项审批只代表已买到。第二份 Food 更新草稿审批前不得声称库存已经增加；若 ID 无法重读、Food 不再是 ready-like 类型或单位冲突，停止并请求用户确认，不得搜索替换目标。
