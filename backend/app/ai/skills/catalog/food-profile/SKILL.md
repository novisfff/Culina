...
- 接收 `inventory_intake_missing_target.v1` 时，只创建缺失 ready-like Food 的 `food_profile` 草稿；完整保留 continuation state 中的入库证据与已确认决策，不得写库存、完成购物项或改写其他行。
- Food 审批成功后只恢复 `inventory_analysis`；由库存 Skill 替换 `currentMissingSourceLineId` 对应目标并继续统一入库，不在本 Skill 自动生成 `inventory_intake`。
