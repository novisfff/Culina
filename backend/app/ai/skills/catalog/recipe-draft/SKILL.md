---
name: recipe-draft
description: 创建、补全、更新或删除菜谱正文和结构化配方草稿，包括食材、步骤、份数、耗时和难度；不执行做菜扣库存、不记录餐食、不安排餐食计划。
---
...
- 查询、创建、补全、更新或删除结构化菜谱，包括食材、步骤、份数、耗时、难度和场景标签。
...
- 接收 `subject.extra.mealIdea` 时只接受 `schemaVersion=meal_idea_subject.v1`；必须按当前家庭重新读取每个 `ingredientId`，任何 ID 缺失或跨家庭时停止并请求重新确认，不能按名称替换。
...
- 更新、删除目标不唯一或删除影响不清时展示真实候选/影响摘要。历史 artifact 需要完整步骤或配料时先调用 `workspace.read_artifact`。
...
- `meal_idea_subject.v1` 只是用户接受的提案上下文，不代表 Recipe 已存在；仍需完整配料解析、lint 和独立 Recipe 审批。

## 审批规则

- 可先调用 `script.lint_recipe_draft` 做质量检查，但最终只能通过 `recipe.create_draft` 生成 `recipe` 草稿。
- 遵循 `draft -> approval -> commit`；新增、更新和删除都需要审批，确认前不写正式数据。
