...
- `intake_preview`：冰箱照片或小票由视觉模型识别文字和物品，但必须先通过 `ingredient.search` 或 `ingredient.resolve_candidates` 取得真实 Ingredient ID，再调用 `inventory.preview_intake_candidates` 生成可审阅候选卡。
...
- 候选项必须使用当前家庭真实 `ingredientId`；无法解析的名称只能进入 `unresolvedLabels`，不得伪造 ID 或混入候选行。

## 候选处理

- Ingredient 缺失时不自行创建；说明缺口并进入 `missing_ingredient` handoff。
- 候选卡本身不写库存。用户点击确认后会在新用户轮次的 `subject.extra.intakeCandidates` 中带回所选真实 ID、数量和单位，并在 `unresolvedLabels` 保留未匹配名称；本 Skill 必须重新读取并校验，先逐项处理缺失名称，之后才可生成 `inventory_operation` 草稿。
...
- 接收 `shopping_completed_ingredient` 时，从 `shopping_to_stock.v1` 取得精确 `ingredientId`，重新调用 `ingredient.read_by_id` 校验当前家庭实体；向用户展示 continuation 中的采购数量和单位，再创建第二份 `inventory_operation` 草稿，动作固定为 `restock`。
...
- 图片或小票包含多个缺失 Ingredient 时，`inventory_missing_ingredient.v1` 使用 `currentLabel`、`pendingLabels` 和 `resolvedItems`，每次只补建一个食材；审批成功后恢复候选整理，不能直接入库。
...
- `inventory_intake_candidates` 只是读取型预览；卡片选择不会复用审批 continuation，也不会跳过后续库存草稿审批。
...
- 展示入库候选时明确区分已匹配真实食材与 `unresolvedLabels`，并提示用户核对选择、数量和单位。
