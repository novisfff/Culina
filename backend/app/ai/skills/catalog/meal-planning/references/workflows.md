...
4. 使用真实 `foodId` 和匹配的 `recipeId` 生成完整计划；如果食物不在食物库中，停止当前计划草稿并提示先进入食物资料流程。
5. 缺失食材提醒中有真实食材匹配时绑定 `ingredientId`；没有匹配时只保留名称和数量，不假装已创建食材档案。
6. 调用 `meal_plan.create_draft`。
7. 返回待确认草稿。
