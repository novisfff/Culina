...
- `action` 可以是 `create`、`update` 或 `transition_tracking_mode`；创建时 `quantity_tracking_mode` 只能是 `track_quantity` 或 `not_track_quantity`；普通 `update` 必须保留真实档案当前模式，模式变更必须先读取 `ingredient.read_by_id` 的 `trackingTransitionContext`，再生成专用切换草稿；`default_expiry_mode` 只能是 `days`、`manual_date` 或 `none`。
...
- 更新现有食材时必须原样带回读取到的 `quantity_tracking_mode`；不要用普通档案更新绕过需要库存表示选择的跟踪模式切换。切换到只记有无时必须明确 availability、状态和位置；切换到精确数量时必须确认没有库存，或填写新的初始数量、单位、状态、采购日和位置。
...
- 接收 `inventory_intake_missing_target.v1` 时，只创建缺失 Ingredient 的 `ingredient_profile` 草稿；完整保留 continuation state 中的入库证据与已确认决策，不得写库存、完成购物项或改写其他行。
- Ingredient 审批成功后只恢复 `inventory_analysis`；由库存 Skill 替换 `currentMissingSourceLineId` 对应目标并继续统一入库，不在本 Skill 自动生成 `inventory_intake`。
- 一次缺多个食材时，先说明整体缺口，再逐项创建一个可确认草稿；每次审批只推进一个 Ingredient。

## 审批规则

- 创建、更新和数量记录方式切换只能调用 `ingredient_profile.create_draft`，遵循 `draft -> approval -> commit`。
- `transition_tracking_mode` 的 Ingredient/state/batch 版本和 `observed_batches` 由 Tool 根据当前家庭真实数据固化；模型只提供目标模式和 resolution，不能编造或修改并发边界。
...
- 数量记录方式切换前说明当前模式、目标模式和库存表示变化；精确数量转只记有无时说明现有批次保留为历史事实，只记有无转精确数量时说明新 resolution 会成为当前物理库存事实。
