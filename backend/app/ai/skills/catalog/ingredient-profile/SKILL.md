...
- `action` 只能是 `create` 或 `update`；创建时 `quantity_tracking_mode` 只能是 `track_quantity` 或 `not_track_quantity`；普通更新必须保留真实档案当前的 `quantity_tracking_mode`，数量记录方式变更应引导用户使用专用切换流程；`default_expiry_mode` 只能是 `days`、`manual_date` 或 `none`。
...
- 更新现有食材时必须原样带回读取到的 `quantity_tracking_mode`；不要用食材档案审批绕过需要库存处置选择的跟踪模式切换。
